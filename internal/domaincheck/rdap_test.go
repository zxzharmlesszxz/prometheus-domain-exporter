...
func TestNewRDAPExpirationLookupSetsTransport(t *testing.T) {
	...
	lookup := NewRDAPExpirationLookup(timeout)

	if lookup.client.Timeout != timeout {
	...
	if transport.ResponseHeaderTimeout != timeout {
		t.Fatalf("ResponseHeaderTimeout = %s, want %s", transport.ResponseHeaderTimeout, timeout)
	}
...
func TestNewRDAPExpirationLookupUsesDefaults(t *testing.T) {
	...
	if lookup.client.Timeout != DefaultTimeout {
		t.Fatalf("client timeout = %v, want %v", lookup.client.Timeout, DefaultTimeout)
	}
...
func TestRDAPBootstrapFresh(t *testing.T) {
	t.Parallel()
	...
	t.Run("no services", func(t *testing.T) {
		...
		l := newRDAPExpirationLookup(nil, "")
		if l.bootstrapFresh(now) {
			t.Fatal("bootstrapFresh() = true, want false (no services)")
		}
	})
	...
	t.Run("fresh", func(t *testing.T) {
		...
		l := newRDAPExpirationLookup(nil, "")
		l.services = map[string]string{"com": "https://rdap.verisign.com"}
		l.fetchedAt = now
		if !l.bootstrapFresh(now) {
			t.Fatal("bootstrapFresh() = false, want true (just fetched)")
		}
	})
	...
	t.Run("expired", func(t *testing.T) {
		...
		l := newRDAPExpirationLookup(nil, "")
		l.services = map[string]string{"com": "https://rdap.verisign.com"}
		l.fetchedAt = now.Add(-48 * time.Hour)
		l.bootstrapTTL = 24 * time.Hour
		if l.bootstrapFresh(now) {
			t.Fatal("bootstrapFresh() = true, want false (expired)")
		}
	})
	...
	t.Run("zero ttl uses default", func(t *testing.T) {
		...
		l.bootstrapTTL = 0
		if !l.bootstrapFresh(now) {
			t.Fatal("bootstrapFresh() = false, want true (zero TTL uses default)")
		}
	})
}

func TestRDAPStaleServiceURL(t *testing.T) {
	t.Parallel()
	...
	t.Run("no services", func(t *testing.T) {
		...
		l := newRDAPExpirationLookup(nil, "")
		if _, ok := l.staleServiceURL("com"); ok {
			t.Fatal("staleServiceURL() ok = true, want false (no services)")
		}
	})
	...
	t.Run("with services", func(t *testing.T) {
		...
		service, ok := l.staleServiceURL("com")
		if !ok {
			t.Fatal("staleServiceURL() ok = false, want true")
		}
		if service != "https://rdap.verisign.com" {
			t.Fatalf("staleServiceURL() = %q, want %q", service, "https://rdap.verisign.com")
		}
	})
	...
	t.Run("missing tld", func(t *testing.T) {
		...
		l := newRDAPExpirationLookup(nil, "")
		l.services = map[string]string{"com": "https://rdap.verisign.com"}
		if _, ok := l.staleServiceURL("org"); ok {
			t.Fatal("staleServiceURL(org) ok = true, want false (missing TLD)")
		}
	})
}

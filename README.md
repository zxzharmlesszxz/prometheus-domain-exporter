...
If no `--domain.config-file` value is provided, `/etc/prometheus/prometheus-domain-exporter.yml` is loaded when it exists; if it is missing, defaults and flags are used.
...
Configure one or more domains with repeatable `--domain.domain` flags. Data refresh runs through the framework snapshot collector in a background worker; scrapes return the last collected snapshot.
